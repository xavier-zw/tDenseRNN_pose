...
def predict_DHP(model, x_selected, y_selected, ch_idx, imgidx):
    ...
    preds, maxvals = get_final_preds(
        CFG, output.clone().detach().cpu().numpy(), c, s)
...
def generate_target(CFG, joints, joints_vis):
    ...
    target = np.zeros((CFG.NUM_JOINTS,
                       CFG.HEATMAP_SIZE[1],
                       CFG.HEATMAP_SIZE[0]),
                      dtype=np.float32)
    ...
    for joint_id in range(CFG.NUM_JOINTS):
        feat_stride = [a / b for a, b in zip(CFG.IMAGE_SIZE, CFG.HEATMAP_SIZE)]
...
def get_final_preds(batch_heatmaps,rectangle):
    coords, maxvals = get_max_preds(batch_heatmaps)

    heatmap_height = batch_heatmaps.shape[2]
    heatmap_width = batch_heatmaps.shape[3]
    preds = coords.copy()
    preds[0, :, 0] = preds[0, :, 0] * ((rectangle[1] - rectangle[0]) / heatmap_width) + rectangle[0]
    preds[0, :, 1] = preds[0, :, 1] * ((rectangle[3] - rectangle[2]) / heatmap_height) + rectangle[2]
...
def accuracy(output, target, hm_type='gaussian', thr=0.5):
    ...
    for i in range(output.shape[0]):
        oks += [compute_kpts_oks(pred[i], target[i])]
    ...
    if cnt != 0:
        acc[0] = avg_acc
    return acc, avg_acc, cnt, pred, oks
...
def compute_kpts_oks(dt_kpts, gt_kpts):
    ...
    scale = (xg.max() - xg.min()) * (yg.max() - yg.min())
    ...
    dx = xd - xg
    dy = yd - yg
    e = (dx**2 + dy**2) / variances / (scale+np.spacing(1)) / 2
...
class JointsMSELoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        temporal = output.size(1) #- 1
        num_joints = output.size(2)
        #heatmaps_init_pred = output[:, 0]
        #heatmaps_init_gt = target[:, 0]
        #output = output[:, 1:]
        heatmaps_pred = output.reshape((batch_size*temporal, num_joints, output.size(3), output.size(4)))
        heatmaps_gt = target.reshape((batch_size*temporal, num_joints, output.size(3), output.size(4)))
        loss = 0

        for idx in range(num_joints):
            heatmap_pred = heatmaps_pred[:, idx].squeeze()
            heatmap_gt = heatmaps_gt[:, idx].squeeze()
            #heatmap_init_pred = heatmaps_init_pred[:, idx].squeeze()
            #heatmap_init_gt = heatmaps_init_gt[:, idx].squeeze()
            loss += 0.5 * self.criterion(heatmap_pred, heatmap_gt) #+ 0.5 * self.criterion(heatmap_init_pred, heatmap_init_gt))
