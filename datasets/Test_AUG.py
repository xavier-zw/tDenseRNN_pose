...
class Test_AUG(Dataset):
    ...
    def gen_temporal_dir(self):
        for data_dir_name, label_dir_name in zip(self.data_dir_list, self.label_dir_list):
            ...
            if self.CFG.temporal_stride:
                for i in range(0, label_num, self.CFG.temporal_stride):
                    tmp1 = []
                    tmp2 = []
                    try:
                        for k in range(self.temporal):
                            imgs = os.path.join(data_dir_name, label_image_name_list[k+i][:-4]+".npy")
                            lables = os.path.join(label_dir_name, label_image_name_list[k+i])
                ...
                for i in range(label_num // self.temporal):
                    ...
                    try:
                        for k in range(self.temporal * i, self.temporal * (i + 1)):
                            imgs = os.path.join(data_dir_name, label_image_name_list[k][:-4]+".npy")
    ...
    def aug(self, img_path_list, point_list):
        angle = np.zeros(4)
        image_list, label_list = [], []
        if self.train and np.random.rand() > 0.5 and "point_rotate" in self.aug_method:
            angle = np.radians((2*np.random.random(4) - 1)*90)
        for path, label in zip(img_path_list, point_list):
            data = np.load(path)
            image, label = random_rotate_point_img(data, label.numpy(), angle)
    ...
    def __getitem__(self, idx):
        ...
        img_path_list = self.temporal_dir[idx].copy()
        joint_list = self.labels[idx].copy()
        image_list, joint_list = self.aug(img_path_list, joint_list)

        margin = 100

        if self.train and np.random.rand() > 0.5 and "random_scale" in self.aug_method:
            margin = np.random.randint(50, 200)
        ...
        min_v = min_v - (margin if min_v > margin else min_v)
        min_u = min_u - (margin if min_u > margin else min_u)
        max_v = max_v + (margin if max_v < self.CFG.SOURCE_SIZE[1] else self.CFG.SOURCE_SIZE[1] - max_v)
        max_u = max_u + (margin if max_u < self.CFG.SOURCE_SIZE[0] else self.CFG.SOURCE_SIZE[0] - max_u)
        degree = np.random.randint(0, 4)
        for k in range(self.temporal):
            data_image = image_list[k]
            joint = joint_list[k]
            ...
            if self.train and degree and "image_rotate" in self.aug_method:
        ...
        if self.train and np.random.rand() > 0.5 and "random_flip" in self.aug_method:
            inputs = inputs.flip(dims=[0])
            targets = targets.flip(dims=[0])
            target_weights = target_weights.flip(dims=[0])
