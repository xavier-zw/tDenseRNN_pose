...
class PoseResNet(nn.Module):
    ...
    def __init__(self, block, layers, **kwargs):
        ...
        self._myparams = nn.Parameter(torch.stack([torch.randn([64, 64]) for i in
                                                   range((self.temporal * (self.temporal - 1)) // 2)]).unsqueeze(dim=0))
        # lstm
        self.outclass = 13
    ...
    # def _weight1(self, x):
    #     x = torch.mul(x, self.params1)
    #     return x
    #
    # def _weight2(self, x):
    #     x = torch.mul(x, self.params2)
    #     return x
    #
    # def _weight3(self, x):
    #     x = torch.mul(x, self.params3)
    #     return x
    #
    # def _weight4(self, x):
    #     x = torch.mul(x, self.params4)
    #     return x
